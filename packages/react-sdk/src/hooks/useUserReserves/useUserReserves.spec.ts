...
import {
  renderHook,
  waitFor,
  mockReservesRpcResponse,
  mockUserReservesHookCall,
} from "@/test";
    ...
    it("wait for data - should return reserves with the correct keys and types", async () => {
      ...
      await waitFor(
        () => {
          ...
          expect(result.current.data?.formattedReserves).toEqual(
            mockUserReservesHookCall.formatted.expectedGeneralData
          );
          result.current.data?.formattedReserves.userReservesData.map(
            (reserve) => {
              expect(reserve).toEqual(
                expect.objectContaining(
                  mockUserReservesHookCall.formatted.expectedUserReserveData
                )
              );
            }
          );
